					...
					<div class="row">
						...
						<div class="col-lg-4">
							...
							<table  class="table  borderless"  width="100%" border="0" >
								<tr>
									<td>
										<a href="javascript:void(0)" type="button" class="btn btn-success  btn-flat  btn-lg btnAssigngroupleader">
											 ASSIGN GROUP LEADER
										</a>
									</td>
								</tr>
	  ...
	  <div class="modal-dialog modal-lg">
		...
		<div class="modal-content">
		  <div class="modal-header">
			...
			<h4 class="modal-title">Assign Group Leader</h4>
		  </div>
		  <div class="modal-body">
			
			<div class="box-body">
			
                <form id="LeaderDetail">
				@csrf
					...
					<table  class="table  borderless"  width="100%" border="0" >
						...
						<tr>
							  <td>Assign Leader</td>
							  <td> 	
							   <input type="text" class="form-control" id="search_leader" name="search_leader" />
							  </td>
							  <td><button type="button"   class="btn btn-success btn-primary  btn-flat" id="btnSearchLeader" >SEARCH</button></td>
						</tr>
		  ...
		  <div class="modal-footer">
			...
			<button type="submit" form="LeaderDetail"   class="btn btn-success btn-primary  btn-flat submitLeader" disabled>Save</button>
		  </div>
...
<script type="text/javascript">
$(document).on('click', '.btnAssigngroupleader', function(e){
e.preventDefault(); 
...
$('#btnSearchLeader').click(function() {
	...
	if(city_val == "" || barangay_val == "" || coordinator_val == "")
	{
		
		swal({
				type:'warning',
				title:"",
				text:"Please Select City , Barangay and Coordinator"
			})
		...
		return false;
			...
			$(".submitLeader").attr("disabled", true);	
			 ...
			 $('#AssignTable').DataTable({ 
				...
				"processing": true, //Feature control the processing indicator.
				"ajax": {
					"url": "{{asset('searchAvailableLeader')}}",
					"data" : {_token: "{{csrf_token()}}",city:city_val,barangay:barangay_val,search:search_leader},
					"type": "post",
				},
				"bFilter": false,
				 "scrollX":  true,
...
$('#AssignTable').on('change', 'input[name="selected_checkbox"]', function() {
	...
	if ($(this).is(":checked")) 
	{
	  ...
	  $(this).closest('tr').find('td select.seleclValue').removeAttr('disabled');
	   ...
	   $(".submitLeader").attr("disabled", false);
...
<script>
$('#LeaderDetail').on('submit', function(event){
event.preventDefault();
	
	
	var city = $("#city option:selected" ).val();	
	var barangay = $("#barangay option:selected" ).val();
	var coordinator = $("#coordinator option:selected" ).val();
	...
	$.ajax({
		url: "{{ asset('AssignLeader') }}",
		data : {city:city,barangay:barangay,coordinator:coordinator,leader:vin_number},
		headers:{'X-CSRF-TOKEN':$('meta[name="_token"]').attr('content')},
		type : 'POST',
		beforeSend:function(){
...
<!--- DELETE  -->
<script>
$(document).on('click','.btnDeleteLeader', function(e){
e.preventDefault();
	...
	swal({
        type:'question',
        title:'',
        text:'Do you really wish to delete this Leader',
...
$("#print_Datax").attr('action', "{{asset('printleader')}}");
$("#print_Datax" ).submit();
...
$("#print_Datax").attr('action', "{{asset('exportleader')}}");
$("#print_Datax" ).submit();
