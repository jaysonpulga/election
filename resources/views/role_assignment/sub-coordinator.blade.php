					...
					<div class='row'>
						<div class="col-lg-12">
							<span class="pull-right">
									<a href="javascript:void(0)" type="button" class="btn btn-primary  btn-flat assign_sub_coordinator">
		  ...
		  <div class="modal-body">
			<div class="box-body">
					...
					<table  class="table  borderless"  width="100%" border="0" >
						...
						<tr>
							  <td>Assign Sub-Coordinator</td>
							  <td>
								  <div class="form-group">
										 <select class="form-control select2" id="assign_sub_coordinator"  name="assign_sub_coordinator" style="width: 100%;" disabled>
										 </select>
								  </div>
							  </td>
							  <td><button type="button"   class="btn btn-success btn-primary  btn-flat" id="addDataSubcoordinator" >ADD</button></td>
						</tr>
...
function loadMainAccount()
{
	...
	*/
	"ajax": {
		"url": "{{asset('getSubCoordinatorList')}}",
...
<script>
$(document).on('click','.assign_sub_coordinator',function(){
	...
	$('select[id="assign_sub_coordinator"]').attr('disabled', 'disabled').empty();
	$('#coordinator').val("");
...
$('#city_municipality').change(function() {
	...
	if( value == ""){
		...
		$('select[id="assign_sub_coordinator"]').attr('disabled', 'disabled');
		$('#coordinator').val("");
	...
	else
	{
		...
		$.ajax({
			url: "{{ asset('getbarangaybelongtoMunicipality') }}",
			...
			success:function(res){
				...
				$.each(res.data, function(index, value) {
					...
					selectValue += "<option value=\""+value.barangay+"\">"+value.barangay+"</option>";
...
$('#barangay').change(function() {
	...
	else
	{
		...
		$('select[id="assign_sub_coordinator"]').removeAttr("disabled");

		
		$.ajax({
			url: "{{ asset('getUserbelongtobarangay') }}",
			data : {'barangay':value},
			headers:{'X-CSRF-TOKEN':$('meta[name="_token"]').attr('content')},
			type : 'POST',
			datatype : 'JSON',
			success:function(res){
				...
				$('#assign_sub_coordinator').empty().html(selectValue);
...
<script>
function BaranagayExists(barangay) {
...
<script>
$(document).on('click','#addDataSubcoordinator',function(){
	...
	let vin_number = $("#assign_sub_coordinator option:selected" ).val();
	let name = $( "#assign_sub_coordinator option:selected" ).text();
	

	let check = BaranagayExists(barangay);
	if(check == true)
	{
		alert('Barangay already exist in the table');
		return false;
	}
	
	if(vin_number == "" || vin_number == undefined)
	{
		alert('Sub Coordinator cannot be null!');
		return false;
	}
	...
	var value =  {
				  ...
				  "barangay":barangay,
				  "sub_coordinator" : name,
				  "vin_number" : vin_number,
				  "action": '<a href="javascript:void(0)" class="delete" data-vin_number="'+vin_number+'"  title="Delete Sub-coordinator" ><i class="fa fa-fw fa-ellipsis-v"></i></a>',
				};
...
$(document).on('click','#saveData',function(){
	...
	var obj = { subcoordinator : temporary_array	}
	
	$.ajax({
		url: "{{ asset('create_assign_sub_coordinator') }}",
		headers:{'X-CSRF-TOKEN':$('meta[name="_token"]').attr('content')},
		data : obj,
		type : 'POST',
		dataType : 'json',
